...
void dac(unsigned int data1)
{
	...
	upper_bits = (data1/256);					// obtain the upper 4 bits 
	upper_bits = upper_bits | 48;					//first 4 bits are config,DacA/b,(un)buffered,2x/1x,bufferDisabl/
	//now obtain the lower 8 bits
	lower_bits = data1&0x00FF;									// ANDing separates the lower 8 bits
